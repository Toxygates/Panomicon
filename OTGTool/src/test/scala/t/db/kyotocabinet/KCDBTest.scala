...
object KCDBTest extends Matchers {
  ...
  def testExtDb(mdb: MatrixDB[PExprValue, PExprValue], d: ColumnExpressionData) {
    ...
    for {
      s <- samples.par; cvs = d.asExtValues(s)
    } {
...
//    println("Request p: " + ps)
    ...
    val confirm = ps.map(p => {
      ...
      Map() ++ (for {
        s <- d.samples; v = d.asExtValue(s, unpacked)
    ...
    val confirmLookup = Map() ++ confirm2.map(c => c.head.probe -> c)
    val dataLookup = Map() ++ vs.map(v => v.head.probe -> v)

    for (p <- ppacked.par) {
      val up = probeMap.unpack(p)
      val r1 = confirmLookup.get(up)
      val r2 = dataLookup.get(up).map(_.toSeq)
      if (r2 == None) {
        assert (r1 == None)
      } else {
        if (r1 != r2) {
