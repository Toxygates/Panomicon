...
class Probes(config: TriplestoreConfig) extends ListManager(config) {
   ...
   * Format: (id, name, experimental, scores available?, suggested limit, size)
  ...
  def mirnaSources: Iterable[(String, String, Boolean, Boolean, Option[Double], Option[Int])] = {
    val q = s"""$tPrefixes
      ...
      triplestore.mapQuery(q).map(x =>
        (x("g"), x("title"), x("experimental").toBoolean, x("hasScores").toBoolean,
  ...
  def mirnaAssociations(probes: Iterable[Probe], scoreLimit: Option[Double],
    ...
    val queryForFilter = if(queryFromMirna) probes.map("\"" + _.identifier + "\"") 
      else probes.map(p => bracket(p.pack))
