...
public class GVizFactory extends Factory<GDTData, GDTDataset> {
	...
	public GDTDataset dataset(List<ChartSample> samples,
			List<ChartSample> allSamples, String[] categories,
			boolean categoriesAreMins) {
		return new GDTDataset(samples, allSamples, categories,
				categoriesAreMins);
	}

	@Override
	public GVizChartGrid grid(Screen screen, GDTDataset table, List<String> rowFilters,
			List<String> organisms, boolean rowsAreMajors,
			String[] timesOrDoses, boolean columnsAreTimes, int totalWidth) {
		return new GVizChartGrid(this, screen, table, rowFilters,
				organisms, rowsAreMajors, timesOrDoses, columnsAreTimes,
				totalWidth);
	}

	public AdjustableGrid<GDTData, GDTDataset> adjustableGrid(Screen screen, 
