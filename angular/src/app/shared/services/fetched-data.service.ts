...
export class FetchedDataService {
  ...
  private fetchData() {
    ...
    this.backend
      .getDatasets()
      .subscribe((datasets) => this.datasets$.next(datasets));

    // fetch batches for selected dataset
    this.userData.selectedDataset$
      .pipe(
    ...
    combineLatest([this.samples$, this.sampleFilters$])
      .pipe(
        pairwise(),
          ...
          if (samples2 == null || filters2.length == 0) {
            ...
            const filteredSamples = samples2.filter((sample) =>
              filters2.every(
                (filter) =>
                  filter.attribute &&
                  filter.passesFilter(sample[filter.attribute])
              )
            );
    ...
    this.userData.selectedBatch$
      .pipe(
        filter((batchId) => batchId != null),
        switchMap((batchId) => {
          return concat(this.backend.getAttributesForBatch(batchId as string));
        })
      )
    ...
    combineLatest([this.samples$, this.attributeMap$])
      .pipe(
          ...
          if (samples && attributeMap) {
            samples.forEach((sample) => {
              Object.keys(sample).forEach((attributeId) => {
                ...
                if (!found)
                  throw new Error(
                    `Sample had unknown attribute ${attributeId}`
                  );
  ...
  fetchAttribute(attribute: Attribute): void {
    ...
    this.backend
      .getAttributeValues(
        ...
        [this.userData.selectedBatch$.value as string],
        [attribute.id]
      )
      .subscribe((result) => {
