...
describe('SampleFilter', () => {
  ...
  it('should correctly test value contains argument', () => {
    ...
    expect(filter.passesFilter('23')).toBeTrue();
    expect(
      filter.passesFilter('2 and a 3 here but not the other thing')
    ).toBeFalse();
  });
  ...
  it('should only validate numbers for < and >', () => {
    [SampleFilterType.LessThan, SampleFilterType.GreaterThan].forEach(
      (type) => {
        filter.type = type;
  ...
  it('should only validate integers for <=, >=, ==, and !=', () => {
    [
      SampleFilterType.LessThanOrEqualTo,
      SampleFilterType.GreaterThanOrEqualTo,
      SampleFilterType.EqualTo,
      SampleFilterType.NotEqualTo,
    ].forEach((type) => {
      filter.type = type;
      filter.parameter = undefined;
      expect(filter.validateParameter()).toBeFalse();
  ...
  it('should validate numeric and non-numeric strings for contains, does not contain, and alphabetically before/after', () => {
    [
      SampleFilterType.Contains,
      SampleFilterType.DoesNotContain,
      SampleFilterType.AlphabeticallyBefore,
      SampleFilterType.AlphabeticallyAfter,
    ].forEach((type) => {
      filter.type = type;
      filter.parameter = undefined;
      expect(filter.validateParameter()).toBeFalse();
