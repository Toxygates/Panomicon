...
export class SampleTableHelper {
  ...
  formatterForFilters(filters: SampleFilter[]): Tabulator.Formatter {
    return (
      ...
      const color = filters.every((filter) => filter.passesFilter(value))
        ? 'blue'
        : 'red';
  ...
  createColumnForAttribute(attribute: Attribute): Tabulator.ColumnDefinition {
    ...
    const filtersForColumn = this.filters$.value.filter(
      (filter) => filter.attribute == attribute.id
    );
  ...
  updateColumnFormatters(tabulator: Tabulator | undefined): void {
    ...
    columns?.forEach((column) => {
      ...
      const filtersForColumn = this.filters$.value.filter(
        (filter) => filter.attribute == definition.field
      );
      const formatter =
        filtersForColumn.length > 0
          ? this.formatterForFilters(filtersForColumn)
          : 'plaintext';
      void column.updateDefinition({
  ...
  static groupHeader(
    ...
    return (
      ...
      if (group.getParentGroup()) {
        ...
        itemWord = ' sample';
        if (
          value != (group.getParentGroup() as Tabulator.GroupComponent).getKey()
        ) {
